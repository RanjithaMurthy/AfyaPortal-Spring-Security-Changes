...
afyaApp.controller('AppointmentController', ['$scope', '$http', '$stateParams','$state', '$location',  function ($scope, $http, $stateParams,$state, $location) {
    ...
    $scope.onDisplayDoctorsClick = function (clinicId, location) {
        //alert('Inside onDisplayDoctorsClick');
    ...
    $scope.onBookAppointmentClick = function (doctor, appointmentDate, timeSlot) {
        //alert('clinicId=' + clinicId + ' providerId=' + providerId + ' appointmentDate=' + appointmentDate + ' timeSlot=' + timeSlot);
        // Checking for user role[Only Patient Can Book Appointment]
    ...
    function ValidatePatientLogedInStatus(clinicId, providerId, appointmentDate, timeSlot) {
        if($scope.username != undefined || $scope.username != null){
            ...
            /*$http.get('/afya-portal/anon/getPatientByUsername?username=' + $scope.username).success(function (data) {
                ...
                $scope.appointmentConfData = { firstName: data.firstName, middleName: data.middleName, lastName: data.lastName, email: data.emailId, mobile: data.mobileNumber, clinic: $scope.clinicName, doctor: $scope.doctorName, slot: $scope.appointmentDateSelected + ' (' + $scope.appointmentSlotSelected + ')', tariff: $scope.providerTariff, tariffTotal: $scope.providerTariffTotal };
                //$scope.appointmentConfData = { firstName: "pat", middleName: null, lastName: 'two', email: "pttwo@email.com", mobile: '1111000011', clinic: $scope.clinicName, doctor: $scope.doctorName, slot: $scope.appointmentSlotSelected, tariff: $scope.providerTariff };
    ...
    $scope.onRegisterClick = function (patientRegisterData){
        ...
        $http({
            ...
            $("#modalFullRegistration").modal("hide");*/
            $http({
                url : '/afya-portal/anon/persistUserLoginFacilityAssociation?userName='+$scope.username+'&tenantId='+$scope.clinicId+'&facilityType=CLINIC',
                method : 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept' : 'text/plain'
    ...
    function BookAppointmentWithSuccessPayment() {
        ...
        $http({
            ...
            sweetAlert({
                    title: "Success",
                    text: 'Thanks for being part of Afya, the scheduled appointment has been booked. Please view in My Account > Upcoming services',
                    type: "success"//error
                },
            function () {
    ...
    function SetPackageServiceFilter(visitType){
        switch(visitType) {
            ...
            //Home Pharmacy - Refill of Prescription , Appointment  Request - Doctors
        }
    }
    ...
    $scope.appliedClass = function() {
        ...
        if ($scope.appointmentVisitType === "" || $scope.appointmentVisitType === undefined) {
            return "col-sm-9 appointments";// two columns for doctors
        } else {
            return "col-sm-5 appointments"; // default
        }
    }
