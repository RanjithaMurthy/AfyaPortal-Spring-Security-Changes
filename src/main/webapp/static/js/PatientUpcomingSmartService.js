...
afyaApp.controller('UpcomingSmartServices', ['$scope', '$http', '$window', '$state', function($scope, $http, $window, $state){
    ...
    function GetUpcomingAppointmentsByAfyaId(afyaId){
        $http.get('/afya-portal/getUpcomingAppointments').success(function(data){
            $scope.upcomingServiceList = data;
            /*$scope.upcomingServiceList = $.map($.grep(data, function (data, idy) {
                return (data.clinicId != null);
            }), function (data, idy) {var obj = $.extend({}, data);  obj.appointmentDateTime = GetDateObjectFromStringDate(data.appointmentDate, data.appointmentSlot); return obj; });*/
            console.log($scope.upcomingServiceList);
        }).error(function(){
            sweetAlert("error fetching data");
        });
    }
    ...
    function GetPharmacyOrdersFromAfyaIdOpenOrdersOnly(afyaId){
        $http.get('/afya-portal/getPharmacyOrdersFromAfyaId?openOrdersOnly=TRUE').success(function(data){
            $scope.pharmacyOrderList = data;
            console.log($scope.pharmacyOrderList);
        }).error(function(){
        ...
        if($scope.validationResult(schedule)){
            ...
            $http({
                ...
                if (data.indexOf("cannot be rescheduled") > -1) {
                    ...
                    sweetAlert({
                        title: "Success",
                        text: "Hi, your appointment has been rescheduled.",
                        type: "success"//error
                    },
                    function () {
    ...
    function GetDateObjectFromStringDate(appointmentDate, timeSlot){
        var result = null;
        var data = appointmentDate.split('-');
        var timeslotArray = timeSlot.split('-');
        var newDate = new Date(new Date(data[2], (data[1] - 1).toString(), data[0], timeslotArray[0].trim().split(':')[0], timeslotArray[0].trim().split(':')[1], 0, 0));
        ...
        if($scope.validationResult(schedule)){
            ...
            function(){
                $http({
                    ...
                    //url: '/afya-portal/anon/cancelAppointmentForAGivenTenantDateAndDoctor?tenantId=' + schedule.clinicId+'&scheduleId='+schedule.scheduleId,
                    method:"POST",
                    headers: {
                        'Content-Type': 'application/json',
                        'accept' : 'application/json'
    ...
    $scope.showModalForReschedule = function(schedule, index, lengthOfArray){
        ...
        $http.get('/afya-portal/anon/validateRcmForReschedule?clinicId='+schedule.clinicId+'&scheduleId='+schedule.scheduleId).success(function(data) {
            if(!data.isRescheduleAllowed){
                sweetAlert({ title: "Alert", text: data.messageToDisplay, type: "warning" });
            }else{
    ...
    function paymentPollCallbackCB(result) {
        ...
        /*if ((result.status == "Declined") || (result.status == "Timed Out")) { // NOTE: THIS IS FOR PRODUCTION
            ...
            PaymentConfirmation(result.trackId, result.paymentId);
        }
    ...
    function PaymentConfirmation(paymentTrackId, paymentId) {
        ...
        $http({
            ...
            url : $scope.clinicBaseURL + '/clinicMaster/updateScheduleTentativeGenerateInvoice?clinicId=' + $scope.scheduleToPay.clinicId + '&scheduleId=' + $scope.scheduleToPay.scheduleId + '&paymentId=' +  paymentId,
            method : "POST",
            headers : {
                'Content-Type': 'application/json',
                'accept' : 'text/plain'
            ...
            sweetAlert({
                title: "Success",
                text: 'Thanks, your appointment has been booked successfully <br> Please refer the Payment Track ID <strong>' + paymentTrackId + '</strong> in future communication',
                type: "success",
                html: true,//error
            },
            function () {
   ...
   ////////////// PAYMENT GATEWAY INTEGRATION RELATED SERVICE CALLS /////////////////
    ...
    {
        $scope.afyaPatientPolicyForServiceHandler.showPopup('Home Pharmacy - Refill of Prescription', function(){onPaymentPharmacyOrderClickInternal();});
    ...
    $scope.onPaymentModeSelectionClick = function(mode){
        ...
        if($scope.PayModeFor == "HomePharmacy"){
            ...
            var timeSlots = GenerateStartTimeAndEndTime(schedule.appointmentSlot, schedule.appointmentDate);
            options = {
                afyaId: schedule.afyaId,
                ...
                doctorId: schedule.doctorId,
                transactionType:schedule.visitType,
                apptSlot: timeSlots['start'],    // kannan 2015-11-07 - to ensure the appointment date and time are correctly recorded in PaymentTransaction table        // schedule.appointmentSlot,
    ...
    function pharmacyOrderPaymentPollCallbackCB(result) {
        ...
        /*if ((result.status == "Declined") || (result.status == "Timed Out")) { // NOTE: THIS IS FOR PRODUCTION
            ...
            CompleteOrderConfirmation(result.trackId);
            /*alert('Success Payment.');*/
            //window.location = "Patient-upcoming-smart-services.html";
        }
        else if(result.status == "Waiting"){
    ...
    function CompleteOrderConfirmation(paymentTrackId){
        ...
        //$http.get('/afya-portal/anon/createActivePrescriptionPayment?clinicId='+ $scope.order.clinicId +'&orderId='+ $scope.order.orderId + '&afyaId=' + $scope.afyaId ,{headers: {'accept': 'text/plain'}}).success(function(data){
        $http.get($scope.clinicBaseURL + '/clinicMaster/createActivePrescriptionPayment?clinicId='+ $scope.order.clinicId +'&orderId='+ $scope.order.orderId + '&afyaId=' + $scope.afyaId ,{headers: {'accept': 'text/plain'}}).success(function(data){
            ...
            sweetAlert({
                title: "Order Status",
                text: "Order Placed Successfully <br> Please refer the Payment Track ID <strong>" + paymentTrackId + "</strong> in future communication",
                type: "success",//error
                html: true,//error
            },
    ...
    $scope.onReceivedPharmacyOrderClick = function(order, index) {
        ...
        //$http.post('/afya-portal/anon/completeOrder?clinicId='+ order.clinicId +'&orderId='+ order.orderId + '&afyaId=' + $scope.afyaId ).success(function(data){
    ...
    $scope.onCancelPharmacyOrder = function(order, index, lengthOfArray){
        ...
        if($scope.validateOrderResult(order)){
            ...
            //$http.post('/afya-portal/anon/cancelOrder?clinicId='+ order.clinicId +'&orderId='+ order.orderId + '&afyaId=' + $scope.afyaId ).success(function(data){
    ...
    function getAvailableTimeSlotsByDate(clinicId, providerId, appointmentDate, visitType) {
        ...
        if(appointmentDate != undefined){
            ...
            //$http.get('/afya-portal/anon/getAvailableTimeslotsForAGivenTenantDateAndDoctor?tenantId=' + clinicId + '&doctorId=' + providerId + '&appointmentDate=' + appointmentDate + '&visitType=' + visitType).success(function (data) {
                angular.forEach(data, function(eachData){
    ...
    $scope.onConfirmationBackClick = function (){
        ...
        $scope.afyaPatientPolicyForServiceHandler.showPopup('Appointment  Request - Doctors', function(){
            $("#modalConfirmation").modal("show");
        });
    }
