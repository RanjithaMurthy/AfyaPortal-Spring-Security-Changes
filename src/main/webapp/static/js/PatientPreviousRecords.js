...
afyaApp.controller('PatientPrescriptionHistoryController', ['$scope', '$http', function($scope, $http){
    $scope.afyaId = null;
    ...
    angular.element(document).ready(function(){
        ...
        $http.get('/afya-portal/static/application-deployment.properties').then(function(response){
            ...
            //$http.get('/afya-portal/anon/getPatientByUsername?username='+username).success(function(data){
                ...
                $http.get('/afya-portal/getPrescriptionInvoiceDetailsForGivenPatient').success(function(data){
    ...
    $scope.onPrintPrescriptionContentClicked = function(prescription, divName)
    {
        $scope.prescription = prescription;
        if($scope.prescription.invoiceId != null) {
            $http.get('/afya-portal/anon/getPrescriptionInvoiceLineItemDetailsForGivenPatient?tenantId='+$scope.prescription.pharmacyId+'&invoiceId='+$scope.prescription.invoiceId).success(function(data){
...
afyaApp.controller('PatientInvoiceHistoryController', ['$scope', '$http', function($scope, $http){
    ...
    angular.element(document).ready(function(){
        $http.get('/afya-portal/static/application-deployment.properties').then(function(response){
            ...
            //$http.get('/afya-portal/anon/getPatientByUsername?username='+username).success(function(data){
                $scope.afyaId = data.afyaId;
                $http.get('/afya-portal/getInvoiceRecordsConsolidated?fromDate=&toDate=&clinicName=&doctorName=&providerType=&invoiceNumber=').success(function(data){
    ...
    $scope.onPrintInvoiceContentClicked = function(invoice, divName)
    {
        $scope.invoice = invoice;
        if($scope.invoice.ID != null) {
            $http.get('/afya-portal/anon/getInvoiceLineItemsForGivenInvoiceFromGivenTenant?tenantId='+$scope.invoice.CLINIC_ID+'&invoiceId='+$scope.invoice.ID).success(function(data){
                ...
                $.removeCookie("invoice", { path: '/afya-portal'});
                $.cookie("invoice",JSON.stringify($scope.invoice));
                $.removeCookie("invoiceLineItems", { path: '/afya-portal'});
                $.cookie("invoiceLineItems",JSON.stringify($scope.invoiceLineItems));
                window.open('/afya-portal/web_pages/member_area/patient/PrintInvoiceHistory.html', '_abc');
